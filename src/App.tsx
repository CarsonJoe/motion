...
// How much of the page the drawer leaves showing. Matches --drawer-peek.
...
export default function App() {
  ...
  const dragDrawer = (event: React.PointerEvent) => {
    ...
    const settle = (target: number) => {
      ...
      shell.classList.add('drawer-settling')
      window.setTimeout(() => setMenuOpen(false), MOBILE_SLIDE_MS)
      window.setTimeout(() => shell.classList.remove('drawer-settling'), MOBILE_SLIDE_MS + 180)
    }
  ...
  const hasPage = Boolean((activeId || landing) && !exitingToList)
