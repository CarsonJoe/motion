...
// A thin, muted spinner keeps long background work legible without competing
// with the document. The visible mark is decorative; assistive technology gets
// the scope-specific label supplied by its caller.
function SyncSpinner({ label, announce = false }: { label: string; announce?: boolean }) {
  return <span className="sync-status" aria-label={label} {...(announce ? { role: 'status', 'aria-live': 'polite' as const } : {})}><span className="sync-spinner" aria-hidden="true" /></span>
}
...
function ReviewPanel({ title, detail, notes, activeId, busy, keepLabel, deleteLabel, selectedIds, onToggleSelected, onOpen, onKeep, onDelete, onNotNow }: {
  title: string; detail: string; notes: Note[]; activeId: string | null; busy: boolean
  keepLabel: string; deleteLabel: string; selectedIds?: ReadonlySet<string>
  onToggleSelected?: (id: string) => void; onOpen: (note: Note) => void
  ...
  return <div className="trash-view review-view" role="dialog" aria-label={title}>
    ...
    <div className="trash-list">{notes.map((note) => { const selected = selectedIds?.has(note.id) ?? false; return <div key={note.id} className={`trash-item ${note.id === activeId ? 'active' : ''} ${selected ? 'selected' : ''}`}><button className="trash-open" onClick={() => onOpen(note)}><span className="trash-title">{note.title || 'Untitled'}</span><span className="trash-when">Open to review</span></button>{onToggleSelected && <button className="review-select" aria-label={`${selected ? 'Exclude' : 'Include'} ${note.title || 'Untitled'} ${selected ? 'from' : 'in'} merge`} aria-pressed={selected} disabled={busy} onClick={() => onToggleSelected(note.id)}><span>{selected && <svg viewBox="0 0 16 16" aria-hidden="true"><path d="m4 8.5 2.5 2.5L12 5.5" /></svg>}</span></button>}</div> })}</div>
...
function NoteTreeNode({ note, depth, ...shared }: NoteTreeShared & { note: Note; depth: number }) {
  ...
  const resourceRoot = Boolean(note.shareId) && parent?.shareId !== note.shareId
  ...
  return <div className="page-tree-item">
    <div data-drop-id={dragEnabled ? note.id : undefined} className={`page-row ${activeId === note.id ? 'active' : ''} ${dragging ? 'dragging' : ''}`}>{hasChildren ? <button className="page-toggle" style={{ marginLeft: 5 + depth * 16 }} aria-label={`${expanded ? 'Collapse' : 'Expand'} ${note.title || 'Untitled'}`} aria-expanded={expanded} onClick={() => onToggleExpanded(key, expanded)}><svg viewBox="0 0 16 16" aria-hidden="true"><path d={expanded ? 'm4 6 4 4 4-4' : 'm6 4 4 4-4 4'} /></svg></button> : <span className="page-toggle-spacer" style={{ marginLeft: 5 + depth * 16 }} />}{renamingKey === key
      ...
      : <a className="page-link" href={pageUrl(note.id, note.shareId || null)} draggable={false} onDragStart={(event) => event.preventDefault()} onPointerDown={dragEnabled ? (event) => onDragStart(note, event, scope) : undefined} onClick={(event) => { if (clickSuppressed()) { event.preventDefault(); return } if (event.metaKey || event.ctrlKey || event.shiftKey || event.button !== 0) return; event.preventDefault(); onOpen(note.id) }}><span className="page-link-label">{note.title || 'Untitled'}</span></a>}{resourceRoot && <span className="resource-root-icon" aria-label="Shared page"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><circle cx="17" cy="9" r="2.5"/><path d="M3.5 19c.5-4 2.5-6 5.5-6s5 2 5.5 6M14 14c3.5-.5 5.5 1.2 6 4"/></svg></span>}<button className={`page-menu-button ${menuKey === key ? 'menu-open' : ''}`} aria-label={`Page options for ${note.title || 'Untitled'}`} aria-haspopup="menu" aria-expanded={menuKey === key} onClick={(event) => onToggleMenu(key, event.currentTarget)}><MoreHorizontalIcon /></button>{previewParentId === note.id && <DropLine depth={depth + 1} />}</div>
...
export default function App() {
  ...
  const [anonReviewSelectedIds, setAnonReviewSelectedIds] = useState<Set<string>>(() => new Set())
  ...
  useEffect(() => {
    setAnonReviewSelectedIds(new Set())
  ...
  const scopeForNewPage = useCallback(async (parent: Note | null) => {
    ...
    if (accessDefault === 'custom') {
      try {
        const room = await createEmptyNoteRoom(parent.shareId, 'Custom access')
  ...
  useEffect(() => {
    ...
    setPageLinkServices({
      ...
      createPage: async (title, parent) => {
        ...
        const note: Note = { id: uid(), title: title || 'Untitled Note', creatorId: sync.user?.id, parentId: host?.id ?? '', ...destination, deletedAt: 0, updatedAt: Date.now() }
  ...
  // The page creator gets one unobtrusive explanation when the page first
  // becomes shared. Access changes never reset it.
  useEffect(() => {
    if (!activeNote?.shareId || !activeNote.creatorId || activeNote.creatorId !== sync.user?.id) { setScopeBannerNote(null); return }
    const key = `motion-access-scope-seen:${activeNote.id}`
    setScopeBannerNote(localStorage.getItem(key) ? null : activeNote)
  }, [activeNote?.id, activeNote?.shareId, activeNote?.roomId, activeNote?.creatorId, sync.user?.id])

  const createNote = async (parent: Note | null = null) => {
    if (!store) return
    try {
      const destination = await scopeForNewPage(parent)
      const note: Note = { id: uid(), title: 'Untitled Note', creatorId: sync.user?.id, parentId: parent?.id ?? '', ...destination, deletedAt: 0, updatedAt: Date.now() }
      await saveNote(store, note)
      setLanding(null)
  ...
  const runSelectedAnonymousAction = async (work: (ids: ReadonlySet<string>) => Promise<void>) => {
    const selected = new Set(anonReviewSelectedIds)
    if (!selected.size) { await runReviewAction(() => work(selected)); return }
    setReviewBusy(true)
    anonReviewStore?.close()
    setAnonReviewStore(null)
    try {
      await work(selected)
      const remaining = await surveyScope(ANON_SCOPE)
      if (!remaining.notes) { closeReview(false); return }
      const reopened = await openLocalStore(ANON_SCOPE)
      setAnonReviewStore(reopened)
      setAnonReviewNotes(reopened.getSnapshot().filter((note) => !note.deletedAt))
      setAnonReviewSelectedIds(new Set())
      if (reviewPreview?.kind === 'anonymous' && selected.has(reviewPreview.note.id)) setReviewPreview(null)
      // Keep the review surface open. The remaining pages are already the next
      // decision; bouncing through the attention dialog adds no information.
      setMenuOpen(true)
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'Could not finish reviewing these pages')
      // The operation is designed to be atomic per page. Reopen whatever
      // remains so an error does not strand the review with a closed store.
      const remaining = await surveyScope(ANON_SCOPE)
      if (remaining.notes) {
        const reopened = await openLocalStore(ANON_SCOPE)
        setAnonReviewStore(reopened)
        setAnonReviewNotes(reopened.getSnapshot().filter((note) => !note.deletedAt))
      } else closeReview(false)
    } finally { setReviewBusy(false) }
  }
  ...
  const openSharing = () => {
    ...
    ]).then(([workspaceRoster, roomRoster, parentRoster]) => {
  ...
  const accountSyncBusy = useDelayedFlag(!syncNotice && heavySync, BUSY_DELAY_MS, BUSY_MIN_MS)
  // Header status belongs exclusively to the open document. Global inventory,
  // authentication and connection notices live in the sidebar footer.
  const documentSyncBusy = useDelayedFlag(docTransport === 'connecting', BUSY_DELAY_MS, BUSY_MIN_MS)
  ...
  const importMarkdownFiles = async (files: File[], parentId: string) => {
    ...
    try {
      for (const file of files) {
        ...
        const destination = await scopeForNewPage(parent)
        const note: Note = { id: uid(), title, creatorId: sync.user?.id, parentId, ...destination, deletedAt: 0, updatedAt: Date.now() }
        await saveNote(store, note)
  ...
  return <div ref={shellRef} className={`app-shell mobile-${mobileView} ${sidebarOpen ? '' : 'sidebar-collapsed'} ${drag ? 'dragging-page' : ''} ${noSlide ? 'no-slide' : ''} ${drawerDragging ? 'drawer-dragging' : ''} ${drawerSettling ? 'drawer-settling' : ''} ${drawerIconX ? 'drawer-x' : ''}`}>
    ...
    <main ref={setMainEl} onScroll={rememberEditorScroll} onMouseDown={reviewPreview ? undefined : focusEditorCanvas} onPointerDown={reviewPreview ? undefined : swipeOpenDrawer}><div className={`scroll-fade scroll-fade-top ${scrollFade.top ? 'visible' : ''}`} aria-hidden="true" />{reviewPreview ? <ReviewPage preview={reviewPreview} isMobile={isMobile} onBack={() => setMenuOpen(true)} /> : activeNote && !landing ? <>{scopeBannerNote?.id === activeNote.id && <div className="access-scope-banner-wrap"><div className="access-scope-banner" role="status" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}><span><strong>{activeNote.roomId ? 'This page has custom access' : 'This page uses workspace access'}</strong><small>{activeNote.roomId ? 'Only selected workspace members can access it. New subpages inherit this access.' : `Everyone in ${workspaceNameFor(activeNote.shareId)} can access it. New subpages inherit this access.`}</small></span><button onClick={() => { localStorage.setItem(`motion-access-scope-seen:${activeNote.id}`, '1'); setScopeBannerNote(null) }}>Got it</button><button className="new" onClick={() => { localStorage.setItem(`motion-access-scope-seen:${activeNote.id}`, '1'); setScopeBannerNote(null); openSharing() }}>Review access</button></div></div>}<header onMouseDownCapture={handleHeaderMouseDown} className={`editor-header ${!isMobile && !editing ? 'transparent' : ''}`}><div className="editor-header-row"><div className="header-left">{(isMobile || !sidebarOpen) && <button className="show-sidebar-button" aria-label={isMobile ? 'Back to your pages' : 'Open sidebar'} onClick={showSidebar}><SidebarIcon /></button>}{isMobile && activeNote && <span className={`header-title ${scrollY > 100 ? 'visible' : ''}`}>{activeNote.title || 'Untitled'}</span>}</div><div ref={setToolbarHost} className={`editor-toolbar ${!isMobile && !editing ? 'hidden' : ''}`} role="toolbar" aria-label="Formatting tools" /><div className="header-right">{remotePresence.length > 0 && <div className="presence-list" aria-label="Online collaborators">{remotePresence.map((presence) => <span key={presence.presenceId} title={presence.displayName} style={{ background: presence.color }}>{presence.displayName.slice(0, 1).toUpperCase()}</span>)}</div>}{documentSyncBusy && <div className="header-status"><SyncSpinner label="Syncing this document" announce={isMobile} /></div>}<div className="header-actions">{copiedMarkdown && <span className="copied-flash" role="status">Copied</span>}<button className="header-button page-options-button" aria-label="Page options" aria-haspopup="menu" aria-expanded={headerMenuOpen} onClick={(event) => { setHeaderMenuAnchor(event.currentTarget); setHeaderMenuOpen((open) => !open) }}><MoreHorizontalIcon /></button></div></div></div></header><article ref={articleRef} onKeyDownCapture={(event) => { if (activeTrashed && editingKeyPressed(event)) { event.preventDefault(); setRecoverPrompt(activeNote) } }}>{activeTrashed && <div className="trash-banner" role="status"><div className="trash-banner-text"><strong>This page is in Recently deleted</strong><span>{describeRetention(activeNote)} before it is permanently deleted.</span></div>{canWriteNote(activeNote) && <button className="new" disabled={recoverBusy} onClick={() => void recover(activeNote)}>{recoverBusy ? 'Recovering…' : 'Recover'}</button>}</div>}{!isMobile && (() => { const pathHidden = sidebarOpen || breadcrumbs.length < 2; return <div className={`page-path article-path ${pathHidden ? 'hidden' : ''}`} aria-label="Page path" aria-hidden={pathHidden} onMouseDown={(event) => event.stopPropagation()}>{breadcrumbs.map((crumb, index) => <Fragment key={crumb.id}>{index > 0 && <i>/</i>}<span className={index === breadcrumbs.length - 1 ? 'breadcrumb-current' : 'breadcrumb-ancestor'}><button onClick={() => openNote(crumb.id)}>{crumb.title || 'Untitled'}</button></span></Fragment>)}</div> })()}<textarea ref={titleInputRef} aria-label="Page title" className="title" rows={1} readOnly={!canEditActiveNote} value={titleDraft.noteId === activeNote.id ? titleDraft.value : activeNote.title} onChange={(event) => patchTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === 'Tab') { event.preventDefault(); focusEditorBody() } }} placeholder="Untitled Note" />
      ...
      {bodyMounted && showBacklinks && backlinks.length > 0 && <section className="backlinks" aria-label="Backlinks"><h2>Backlinks</h2>{backlinks.map((note) => <button key={note.id} className="backlink" onClick={() => openNote(note.id)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 3v5h5M14 3H6v18h12V8z" /></svg><span>{note.title || 'Untitled'}</span></button>)}</section>}
      </article></> : landing ? <section className="empty access-landing">{!isMobile && !sidebarOpen && <button className="empty-sidebar-open show-sidebar-button" aria-label="Open sidebar" onClick={openSidebar}><SidebarIcon /></button>}<div className="empty-mark"><img src="/pad-icon.svg" alt="" /></div>{(() => {
    ...
    {shareOpen && activeNote && (() => {
      ...
      const includeSubpagesToggle = <label className="include-subpages minor"><span><strong>Include subpages</strong><small>{subpageCount ? `${subpageCount} subpage${subpageCount === 1 ? '' : 's'} beneath this page` : 'No subpages yet'}</small></span><input type="checkbox" checked={includeSubpages} disabled={!subpageCount || inviteBusy} onChange={(event) => setIncludeSubpages(event.target.checked)} /></label>
      ...
      return createPortal(<div className="share-modal-backdrop" role="presentation" onPointerDownCapture={() => controllerRef.current?.setSelection(null)} onMouseDown={() => setShareOpen(false)}><section className="share-modal workspace-share-modal" role="dialog" aria-modal="true" aria-label="Share page" onMouseDown={(event) => event.stopPropagation()}>
        <header className="share-sheet-header">
          {shareView !== 'initial'
            ? <nav className="share-view-tabs" role="tablist" aria-label="Sharing options"><button role="tab" aria-selected={shareView === 'workspace'} className={shareView === 'workspace' ? 'active' : ''} onClick={() => setShareView('workspace')}>People</button><button role="tab" aria-selected={shareView === 'access'} className={shareView === 'access' ? 'active' : ''} onClick={() => setShareView('access')}>Page access</button></nav>
            : inferredDestination.kind === 'new'
              ? <div className="share-header-title"><strong>Create workspace</strong><span>Workspaces are an access boundary that can contain multiple notes. To share a page, it has to be within a workspace.</span></div>
              : <span />}
        ...
        {shareView === 'initial' ? <>
          {inferredDestination.kind === 'new' && <div className="workspace-create"><input aria-label="Workspace name" value={workspaceName} disabled={inviteBusy} onChange={(event) => setWorkspaceName(event.target.value)} placeholder="Workspace name" /></div>}
          ...
          {(inferredDestination.kind === 'new' || inferredDestination.kind === 'existing' && inferredDestination.source !== 'descendant')
            ? <div className="share-modal-actions">{includeSubpagesToggle}<button className="copy-link" onClick={() => setShareOpen(false)}>Cancel</button><button className="new" disabled={inviteBusy || (inferredDestination.kind === 'new' && !workspaceName.trim())} onClick={() => void addPageToWorkspace()}>{inviteBusy ? inferredDestination.kind === 'new' ? 'Creating…' : 'Adding…' : inferredDestination.kind === 'new' ? 'Create workspace' : 'Add to workspace'}</button></div>
            : includeSubpagesToggle}
        </> : shareView === 'access' ? <>
          ...
          <div className="access-choice-list" role="radiogroup" aria-label="Page access">
            {(['workspace', 'parent', 'custom'] as PageAccessMode[]).map((mode) => {
              ...
              const detail = mode === 'workspace' ? 'Everyone invited from the People tab' : mode === 'parent' ? `Same access as ${parent?.title || 'parent'}` : 'Choose people, or nobody for only you'
              return <Fragment key={mode}><button className={`access-choice ${accessMode === mode ? 'selected' : ''}`} role="radio" aria-checked={accessMode === mode} disabled={!canManageWorkspace || inviteBusy} onClick={() => setAccessMode(mode)}><i /><span><strong>{mode[0].toUpperCase() + mode.slice(1)}</strong><small>{detail}</small></span>{audiencePreview(audienceFor(mode))}</button>{mode === 'custom' && accessMode === 'custom' && <div className="custom-member-list"><input className="custom-member-search" type="search" aria-label="Search workspace members" placeholder="Search workspace members" value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} />{activeWorkspaceMembers.map((member) => { const selected = selectedMemberIds.has(member.userId); const name = member.ownerDisplayName || (member.ownerHandle ? `@${member.ownerHandle}` : member.userId.slice(0, 8)); return <button key={member.userId} className={selected ? 'selected' : ''} disabled={member.userId === sync.user?.id} onClick={() => setSelectedMemberIds((current) => { const next = new Set(current); if (selected) next.delete(member.userId); else next.add(member.userId); return next })}><i>{selected ? '✓' : ''}</i><span>{name}<small>{member.role}</small></span></button> })}{activeWorkspaceMembers.length === 0 && <p>No other people in this workspace. Add them from the People tab.</p>}</div>}</Fragment>
            })}
          </div>
          {shareError && <p className="share-error" role="alert">{shareError}</p>}
          <div className="share-modal-actions"><span /><button className="copy-link" onClick={() => setShareOpen(false)}>Cancel</button>{canManageWorkspace && <button className="new" disabled={inviteBusy} onClick={() => void savePageAccess()}>{inviteBusy ? 'Saving…' : 'Save'}</button>}</div>
        </> : <>
          {!canManageWorkspace && <div className="workspace-admin-note">Only workspace admins can make changes.</div>}
          {canManageWorkspace && invitePeopleControls}
          <div className="workspace-roster">{workspaceMembers.map((member) => <div key={member.userId}><span>{member.ownerDisplayName || member.ownerHandle || member.userId.slice(0, 8)}<small>{member.state}</small></span>{!canManageWorkspace || member.role === 'owner' || member.userId === sync.user?.id ? <b>{member.role}</b> : <><select value={member.role} disabled={inviteBusy} onChange={(event) => void changeMemberRole(member, event.target.value as ShareRole)}><option value="admin">Can manage</option><option value="writer">Can edit</option><option value="reader">Can view</option></select><button aria-label="Remove member" disabled={inviteBusy} onClick={() => void removeWorkspaceMember(member)}>×</button></>}</div>)}</div>
          {shareError && <p className="share-error" role="alert">{shareError}</p>}
        </>}
