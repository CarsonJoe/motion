...
export default function App() {
  ...
  return <div ref={shellRef} className={`app-shell mobile-${mobileView} ${sidebarOpen ? '' : 'sidebar-collapsed'} ${drag ? 'dragging-page' : ''} ${noSlide ? 'no-slide' : ''} ${drawerDragging ? 'drawer-dragging' : ''} ${drawerSettling ? 'drawer-settling' : ''}`}>
    ...
    {isMobile && mobileView === 'drawer' && <div className="drawer-close-layer"><button className="show-sidebar-button drawer-close" aria-label="Close page" onClick={() => { clearPendingNavigation(); leaveToList(true) }}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18" /></svg></button></div>}
