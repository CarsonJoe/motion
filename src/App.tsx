...
export default function App() {
  ...
  const [shareDestination, setShareDestination] = useState<'parent' | 'new'>('new')
  ...
  const invite = async () => {
    ...
    try {
      ...
      const destination = !activeNote.shareId && shareDestination === 'parent' && parent?.shareId
  ...
  return <div ref={shellRef} className={`app-shell mobile-${mobileView} ${sidebarOpen ? '' : 'sidebar-collapsed'} ${drag ? 'dragging-page' : ''} ${noSlide ? 'no-slide' : ''} ${drawerDragging ? 'drawer-dragging' : ''} ${drawerSettling ? 'drawer-settling' : ''} ${drawerIconX ? 'drawer-x' : ''}`}>
    ...
    {headerMenuOpen && headerMenuAnchor && activeNote && <HeaderMenu
      ...
      onShare={() => { const parent = notes.find((note) => note.id === activeNote.parentId); setShareDestination(!activeNote.shareId && parent?.shareId ? 'parent' : 'new'); setHeaderMenuOpen(false); setShareOpen(true) }}
