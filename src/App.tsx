...
export default function App() {
  ...
  const loadInvitations = useCallback(async () => {
    ...
    catch { /* invitations refresh again on the next focus */ }
  ...
  const share = async () => {
    if (!store || !activeNote || !sync.connected) return
    setShareOpen(true)
    setSharePreparing(true)
    try { await shareNoteTree(store, activeNote); setActiveId(activeNote.id) }
    catch (error) { setActionError(error instanceof Error ? error.message : 'Could not share this page'); setShareOpen(false) }
    finally { setSharePreparing(false) }
  }

  const invite = async () => {
    if (!activeNote?.shareId || !inviteHandle.trim() || inviteBusy) return
    ...
    try {
      const member = await inviteByHandle(activeNote.shareId, handle, inviteRole)
